...
function createMarble(color, quantity=1, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB03 CRE ' + color + " " + quantity + '" --cn CUST003',
    function (err, data, stderr) {
...
function deleteMarble(color, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB03 DEL ' + color + '" --cn CUST003',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
...
function getMarbleQuantity(color, callback) {
  ...
  cmd.get(
    'bright jobs submit data-set "CUST003.MARBLES.JCL(MARBDB2)" --rff jobid --rft string',
    function (err, data, stderr) {
      if(err){
        callback(err);
      } else {
...
function updateMarble(color, quantity, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB03 UPD ' + color + " " + quantity + '" --cn CUST003',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
